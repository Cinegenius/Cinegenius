...
export default function CreativeSpark() {
  ...
  const generate = async () => {
    ...
    try {
      ...
      if ((e as Error).name !== "AbortError") setError("Etwas ist schiefgelaufen.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <section className="py-20 border-t border-border">
      <div className="max-w-2xl mx-auto px-4 sm:px-6 text-center">
        ...
        <p className="text-text-muted text-sm mb-8 leading-relaxed">
          Gib ein paar Stichpunkte ein — die KI schreibt dir sofort einen Story-Pitch daraus.
        </p>
        ...
        {(output || loading) && (
          <div className="rounded-2xl border border-gold/20 bg-gold-subtle p-6 text-left">
            ...
            {!loading && output && (
              <button onClick={copy} className="mt-4 inline-flex items-center gap-1.5 text-xs text-text-muted hover:text-gold transition-colors">
        ...
        {error && <p className="text-xs text-crimson-light mt-3">{error}</p>}
        <p className="text-[10px] text-text-muted mt-6 opacity-60">Powered by Groq · Llama 3.1 · kostenlos</p>
      </div>
    </section>
  );
}
