...
export async function POST(req: NextRequest) {
  ...
  if (!prompt || typeof prompt !== "string" || prompt.trim().length < 3) {
    return new Response(JSON.stringify({ error: "Bitte gib mindestens ein paar Stichpunkte ein." }), { status: 400 });
  }
  if (prompt.length > 400) {
    return new Response(JSON.stringify({ error: "Zu lang (max. 400 Zeichen)" }), { status: 400 });
  }
